...
rm -rf $JETSON_BUILD_DIR/Linux_for_Tegra/rootfs/dev
mkdir -p $JETSON_BUILD_DIR/Linux_for_Tegra/rootfs/dev
...
case "$JETSON_NANO_BOARD" in
    jetson-nano-2gb)
        ...
        popd > /dev/null
        mv $JETSON_BUILD_DIR/Linux_for_Tegra/tools/jetson.img . > /dev/null
        printf "[OK]\n"
        ;;
